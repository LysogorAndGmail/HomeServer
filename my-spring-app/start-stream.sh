...
while true; do
  ffmpeg -loglevel quiet -nostdin -f alsa -i plughw:0,0 -af "volume=5.0" -acodec pcm_s16le -ac 1 -ar 16000 -f s16le - | \
  ...
  # Если поток порвался, ждем 2 секунды перед повторной попыткой
  sleep 2
done
